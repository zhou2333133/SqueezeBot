    ...
    def test_old_settings_file_is_migrated_to_current_strategy_profile(self) -> None:
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp:
                ...
                self.assertEqual(manager.settings["SCALP_STRUCTURE_TRAIL_BARS"], 20)
                self.assertFalse(manager.settings["SCALP_TP3_AGGRESSIVE_RUNNER"])
                ...
                self.assertEqual(manager.settings["SCALP_TP1_SOFT_BREAKEVEN_PCT"], 0.35)
                self.assertEqual(manager.settings["SCALP_REVERSAL_STOP_SL_FRACTION"], 0.40)
                self.assertEqual(manager.settings["SCALP_WS_STALE_SECONDS"], 90)
                self.assertEqual(manager.settings["SCALP_POSITION_CHECK_INTERVAL_SECONDS"], 10)
                self.assertEqual(manager.settings["SCALP_POSITION_STALE_SECONDS"], 60)
                self.assertTrue(manager.settings["SCALP_SKIP_UNKNOWN_STATE"])
                self.assertEqual(manager.settings["SCALP_MAX_CANDIDATE_AGE_MINUTES"], 180)
                ...
                self.assertTrue(manager.settings["SCALP_BLOCK_TREND_LATE_ENTRY"])
                self.assertEqual(manager.settings["BREAKOUT_MIN_PCT"], 0.15)
                self.assertEqual(manager.settings["BREAKOUT_ATR_MULT"], 0.7)
                self.assertEqual(manager.settings["BREAKOUT_ATR_MIN_PCT"], 0.50)
                self.assertEqual(manager.settings["BREAKOUT_ATR_MAX_PCT"], 2.00)
                ...
                self.assertEqual(manager.settings["CONTINUATION_TAKER_MIN_LONG"], 0.62)
                self.assertEqual(manager.settings["CONTINUATION_TAKER_MIN_SHORT"], 0.52)
                self.assertEqual(manager.settings["CONTINUATION_HOT_TAKER_MIN"], 0.52)
                self.assertEqual(manager.settings["CONTINUATION_HOT_TAKER_MIN_LONG"], 0.60)
                self.assertEqual(manager.settings["CONTINUATION_HOT_TAKER_MIN_SHORT"], 0.52)
                ...
                self.assertEqual(manager.settings["CONTINUATION_ATR_MIN_PCT_LONG"], 0.70)
                self.assertEqual(manager.settings["CONTINUATION_ATR_MIN_PCT_SHORT"], 0.30)
                self.assertEqual(manager.settings["CONTINUATION_ATR_MAX_PCT"], 2.50)
                self.assertEqual(manager.settings["CONTINUATION_ATR_MAX_PCT_LONG"], 2.00)
                self.assertEqual(manager.settings["CONTINUATION_ATR_MAX_PCT_SHORT"], 1.50)
                self.assertEqual(manager.settings["CONTINUATION_MAX_EMA20_DEVIATION_PCT"], 4.00)
                self.assertEqual(manager.settings["CONTINUATION_MAX_EMA20_DEVIATION_PCT_LONG"], 2.50)
                self.assertEqual(manager.settings["CONTINUATION_MAX_EMA20_DEVIATION_PCT_SHORT"], 2.50)
