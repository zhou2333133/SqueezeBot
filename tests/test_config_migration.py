    ...
    def test_old_settings_file_is_migrated_to_current_strategy_profile(self) -> None:
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp:
                ...
                self.assertEqual(manager.settings["SCALP_TP1_RATIO"], 0.40)
                ...
                self.assertEqual(manager.settings["SCALP_TP1_RR"], 1.2)
                ...
                self.assertEqual(manager.settings["BREAKOUT_MIN_VOL_RATIO"], 0.60)
                ...
                self.assertEqual(manager.settings["BREAKOUT_MAX_EMA20_DEVIATION_PCT"], 2.0)
                self.assertTrue(manager.settings["CONTINUATION_PULLBACK_ENABLED"])
                self.assertEqual(manager.settings["CONTINUATION_TAKER_MIN"], 0.55)
                self.assertEqual(manager.settings["CONTINUATION_HOT_TAKER_MIN"], 0.52)
                self.assertEqual(manager.settings["CONTINUATION_MIN_PULLBACK_PCT"], 0.12)
                ...
                self.assertEqual(manager.settings["CONTINUATION_MAX_EMA20_DEVIATION_PCT"], 4.50)
