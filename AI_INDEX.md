...
[yaobi_scanner] 异常雷达 → intelligence 剧本 → AI 终审 → set_opportunity_queue
    ↓                                                │
...
| 妖币扫描效果 | `?detail=full` 的 `妖币扫描.top_candidates / decision_cards`，重点看 `market_stage / trade_permission / case_similarity` |
...
- `trade_permission` 是 scanner 市场结构许可，不等同于自动执行许可；`AMBUSH_WATCH` 只能观察，真正执行仍必须有 `opportunity_permission=ALLOW_IF_1M_SIGNAL` + 1m 本地触发。
...
|---|---|---|---|---|
| `SCALP_BLOCK_TREND_LATE_ENTRY` | True | 新增 | 52 笔复盘里 TREND_LATE 新仓样本为亏损，趋势末端不再开新仓 | 2026-05-01 |
| `BREAKOUT_ATR_MIN_PCT` | 0.50 | 0.30 | 全样本过滤 ATR<0.5 后净值改善，降低低波动假突破/假接力 | 2026-05-01 |
| `BREAKOUT_ATR_MAX_PCT` | 2.00 | 3.00 | ATR>2.0 样本整体拖累，过滤插针剧烈波动 | 2026-05-01 |
| `CONTINUATION_TAKER_MIN_LONG` | 0.62 | 新增 | 顺势回踩多 36 笔胜率 36.1%，多头接力需要更强主动买盘 | 2026-05-01 |
| `CONTINUATION_TAKER_MIN_SHORT` | 0.52 | 新增 | 顺势回踩空 10 笔胜率 70%，保持较灵敏，不误伤空头优势样本 | 2026-05-01 |
| `CONTINUATION_HOT_TAKER_MIN_LONG` | 0.60 | 新增 | HOT 多头也不能低于 60% 主动买盘，避免弱反抽追入 | 2026-05-01 |
| `CONTINUATION_HOT_TAKER_MIN_SHORT` | 0.52 | 新增 | 空头 HOT 继续沿用灵敏阈值 | 2026-05-01 |
| `CONTINUATION_ATR_MIN_PCT_LONG` | 0.70 | 新增 | 只拦截顺势回踩多 ATR<0.7 的规则在复盘模拟中拦掉净亏组合 | 2026-05-01 |
| `CONTINUATION_ATR_MAX_PCT_LONG` | 2.00 | 新增 | 多头接力 ATR 过高时多为插针环境 | 2026-05-01 |
| `CONTINUATION_MAX_EMA20_DEVIATION_PCT_LONG` | 2.50 | 新增 | 多头回踩后仍过度偏离 EMA20 时不再追 | 2026-05-01 |
| `SCALP_STRUCTURE_TRAIL_BARS` | 20 | 14 | TP3/runner 是主要利润来源，放宽结构追踪窗口减少卖飞 | 2026-05-01 |
| `SCALP_TP3_AGGRESSIVE_RUNNER` | False | True | 复盘里 `valid_runner_missed` / `exit_too_early` 都是大盈利组，默认让 runner 多留 | 2026-05-01 |
| `SCALP_TP1_SOFT_BREAKEVEN_PCT` | 0.35 | 0.60 | 保留 TP1 后呼吸空间，但减少软保本回吐成本 | 2026-05-01 |
| `SQUEEZE_TAKER_MIN_LONG/SHORT` | 0.62 | 0.58 | Squeeze 近两包无成交样本，先按实盘安全收紧反打确认 | 2026-05-01 |
| `SCALP_SQUEEZE_OI_REBOUND_PCT` | 0.12 | 0.05 | 轧空/轧多反打要求 OI 企稳更明确，防二次踩踏 | 2026-05-01 |
